...
Drupal.behaviors.paywallMetered = function(){
  var nid = Drupal.settings.paywallMetered.nid;
  var referrers = Drupal.settings.paywallMetered.referrers;
  ...
  if (!isFromReferrer(referrers) && nid) {
    ...
    var meterTimer = Drupal.settings.paywallMetered.timeOut ? Drupal.settings.paywallMetered.timeOut : 5000; // Timer set to 5 seconds.
    setTimeout (function(){
      var limit = Drupal.settings.paywallMetered.limit ? Drupal.settings.paywallMetered.limit : 5;
      var nidCookie = Drupal.settings.paywallMetered.nidCookie ? Drupal.settings.paywallMetered.nidCookie : 'nidlist';
      var wallCookie = Drupal.settings.paywallMetered.wallCookie ? Drupal.settings.paywallMetered.wallCookie : 'limit';
      var timeframe = Drupal.settings.paywallMetered.timeFrame ? Drupal.settings.paywallMetered.timeFrame : 'fixedperiod';
      ...
      if (timeframe == 'fixedperiod'){
        var days = Drupal.settings.paywallMetered.nrOfDays ? Drupal.settings.paywallMetered.nrOfDays : 7;
        date.setTime(date.getTime()+(days*24*3600*1000));
      }
      else {
        var fixedDay = Drupal.settings.paywallMetered.fixedDay ? Drupal.settings.paywallMetered.fixedDay : 'Friday';
        var days = verifyDayLeft(fixedDay, date.getDay());
      ...
      if (cookieArray) {
        ...
        var nidList = cookieArray[0];
        var expiryDate = cookieArray[1];
        ...
        if (verifyNids(nidList, nid) && (nidList.length < limit)) {
          ...
          nidList += ',' + nid;
          $.cookie(
            nidCookie, nidList + '|' + expiryDate, {expires: expiryDate, path: "/"}
          );
        }
      }
      else {
        ...
        $.cookie(
          wallCookie, 'allow', {expires: date, path: "/"}
        );
      }
    }, meterTimer);
  }

  function verifyNids(nidList, nid){
    // Verify that the current node id hasn't been already stored in the cookie.
