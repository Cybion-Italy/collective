...
function s_doPlugins(s) {
	...
	s.prop18 = s.getQueryParam('subjectid');
...
function setEcoMode(s) {
	//Populate the mode
	var scEconMode = s.getQueryParam('mode');
	if (scEconMode) {
		var sc_href = unescape(document.location.href);
		var sc_regex_mode = /economist\.com.+\./i;
		var stringMatch = sc_href.match(scEconMode);

		if (stringMatch) {
			stringMatch = stringMatch.toString();
			s.prop14 = scEconMode + '|' + stringMatch.substring(0, stringMatch.length - 1);
		} else {
			s.prop14 = scEconMode + '|' + sc_href;
		}
	}
}
